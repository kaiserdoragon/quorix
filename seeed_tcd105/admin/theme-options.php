...
<div class="wrap">
 ...
 <h2><?php _e( 'TCD Theme Options', 'tcd-seeed' ); ?></h2> 
 ...
 <div class="updated fade">
  <p><strong><?php _e( 'Updated', 'tcd-seeed' ); ?></strong></p>
 </div>
 <?php endif; ?>
 ...
 <div id="my_theme_option" class="cf">
  ...
  <div id="my_theme_right">
   ...
   <div id="saving_data" style="display:none;"><p><?php _e('Now saving...', 'tcd-seeed'); ?></p></div>
  </div>
...
require get_template_directory() . '/admin/inc/case_study.php';
...
function theme_options_tools_notices() {
	...
	if ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'error' === $tools_result[1] ) {
		...
		if ( isset( $tools_result[2] ) ) {
			...
			switch( $int_import_error ) {
				...
				case 8:
					...
					break;
				case 11:
                  $message = $int_import_error . ' : ' . __( 'File has not been uploaded.', 'tcd-seeed' );
					break;
				case 12:
                  $message = $int_import_error . ' : ' . __( 'There is no file', 'tcd-seeed' );
					break;
				case 13:
                  $message = $int_import_error . ' : ' . __( 'The file extension is not .json', 'tcd-seeed' );
					break;
				case 14:
                  $message = $int_import_error . ' : ' . __( 'Failed to read the file', 'tcd-seeed' );
					break;
				case 15:
                    $message = $int_import_error . ' : ' . __( 'Json decoding failed', 'tcd-seeed' );
					break;
				default :
		...
		echo '<div class="update-message notice notice-error notice-alt"><p><strong>' . sprintf( __( 'Import error. %s', 'tcd-seeed' ), $message ) . '</strong></p></div>';

	// インポート成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings imported', 'tcd-seeed' ) . '</strong></p></div>';

	// リセット成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'reset' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings reset', 'tcd-seeed' ) . '</strong></p></div>';
	}
}
...
/**
 * seeed オプションTools デフォルト画像設定フィルター
 */
function seeed_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			'media_filename' => 'image_1450x850.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x850.gif',
		...
		array(
			...
			'theme_option_keys' => array("archive_blog_header_image","archive_news_header_image","archive_case_study_header_image","archive_service_header_image","archive_faq_header_image","footer_contact_image")
		),
		array(
			'media_filename' => 'image_1450x500.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x500.gif',
			'theme_option_keys' => array("['contents_builder'][1]['item_list'][0]['bg_image2']")
		),
		array(
			'media_filename' => 'image_500x500.png',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_500x500.png',
			'theme_option_keys' => array("['contents_builder'][1]['item_list'][0]['image1']")
		)
	);
}
...
/**
 * seeed オプションTools サンプルカテゴリー
 */
function seeed_theme_options_tools_set_sample_categories( $dp_options ) {
  ...
  // 事例カテゴリー
	for ( $i = 1; $i <= 3; $i++ ) {
		$sample_categories[] = array(
			'name' => __( 'Category', 'tcd-seeed' ) . $i,
			'slug' => 'case_study_category'.$i,
			'taxonomy' => 'case_study_category',
			'description' => __( 'Description will be displayed here.', 'tcd-seeed' ),
			'metas' => array(
...
/**
 * seeed オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function seeed_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログ、お知らせのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_720x460.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_720x460.gif'
	), true );

	// 事例、サービスのアイキャッチ画像
	$case_study_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_800x500.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_800x500.gif'
	), true );
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample post', 'tcd-seeed' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-seeed' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample news', 'tcd-seeed' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-seeed' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample case study', 'tcd-seeed' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-seeed' ),
			'post_name' => 'sample-case_study' . $i,
			'post_status' => 'publish',
			'post_type' => 'case_study',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $case_study_media_id,
			'taxonomies' => array(
				'case_study_category' => array('case_study_category1','case_study_category2','case_study_category3')
			),
			'metas' => array(
				'case_study_name' => __( 'Sample name', 'tcd-seeed' ),
				'case_study_job' => __( 'Job name', 'tcd-seeed' ),
			)
		);
	}
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample service', 'tcd-seeed' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-seeed' ),
			...
			'metas' => array(
				'service_icon' => 'e8b8',
				'service_sub_title' => __( 'Subtitle', 'tcd-seeed' ),
				'service_desc' => __( 'Description will be displayed here.', 'tcd-seeed' )
			)
		);
	}
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample FAQ', 'tcd-seeed' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-seeed' ),
			'post_name' => 'sample-faq' . $i,
			'post_status' => 'publish',
			'post_type' => 'faq',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'taxonomies' => array(
				'faq_category' => array('faq_category1','faq_category2','faq_category3')
			),
		);
	}
...
/**
 * seeed オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function seeed_theme_options_tools_set_sample_menus( $dp_options ) {
	...
	if ( $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach( $nav_menus as $nav_menu ) {
			if ( $nav_menu->name == __( 'Sample menu', 'tcd-seeed' ) )
				return $dp_options;
		}
	}

	// サンプルメニュー作成
	$menu_id = wp_create_nav_menu( __( 'Sample menu', 'tcd-seeed' ) );
...
/**
 * seeed オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function seeed_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		array(
			...
			'params' => array(
        'title' => __('Recommend post', 'tcd-seeed'),
        'post_num' => '3',
		...
		array(
			...
			'params' => array(
        'post_num' => '3',
        'title1' => __('Recent post', 'tcd-seeed'),
        'title2' => __('Recommend post', 'tcd-seeed'),
        'content_type1' => 'post',
        'content_type2' => 'post',
        'post_order1' => 'date',
        'post_order2' => 'rand',
 ...
 * seeed テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'seeed_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'seeed_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'seeed_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'seeed_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'seeed_theme_options_tools_set_sample_widgets', 10, 1 );
