...
<section id="archive_faq">
   ...
   <div id="faq_sort_button" class="swiper-wrapper">
    <?php
         ...
         foreach ( $category as $cat ) :
           $cat_id = $cat->term_id;
    ?>
    <a data-category-id="#ajax_post_cat_<?php echo esc_attr($cat_id); ?>" href="#faq_content<?php echo $i; ?>" class="swiper-slide no_auto_scroll<?php if($i == 1){ echo ' active'; }; ?>" data-filter="#faq_content<?php echo $i; ?>"><?php echo esc_html($cat->name); ?></a>
    <?php $i++; endforeach; ?>
   </div>
  </div>
 ...
 <div id="faq_list_wrap">
  ...
  <?php
       ...
       if ($category) {
         foreach ( $category as $cat ) :
           $cat_id = $cat->term_id;
  ?>
  <div class="ajax_post_list_wrap faq_content<?php if($i == 1){ echo ' active'; }; ?>" id="ajax_post_cat_<?php echo esc_attr($cat_id); ?>">
  ...
  <div class="ajax_post_list_wrap faq_content<?php if($i == 1){ echo ' active'; }; ?>" id="ajax_post_no_cat">

   <div class="faq_content_inner">
    <?php
         // 質問一覧
         $args = array( 'order' => 'DESC', 'orderby' => 'date', 'post_status' => 'publish', 'post_type' => 'faq', 'posts_per_page' => $post_num );
         $post_list = new wp_query($args);
         $all_post_count = $post_list->found_posts;
         if($post_list->have_posts()):
    ?>
