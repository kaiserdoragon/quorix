...
function seo_meta_box() {
  ...
  $post_types = array ( 'post', 'page', 'news', 'case_study','service');
  add_meta_box(
    'show_seo_meta_box',//ID of meta box
    __('SEO', 'tcd-seeed'),//label
...
<div class="tcd_custom_fields">
 ...
 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Title tag', 'tcd-seeed' ); ?></h3>
  <input type="text" name="tcd-w_meta_title" value="<?php if(!empty($seo_title)){ echo esc_attr($seo_title); }; ?>" style="width:100%" />
 </div><!-- END .content -->

 <div class="tcd_cf_content">
  <h3 class="tcd_cf_headline"><?php _e( 'Meta description tag', 'tcd-seeed' ); ?></h3>
  <p><?php printf(__('Recommended number of characters is %s.', 'tcd-seeed'), '180'); ?></p>
  ...
  $blog_label = $blog_page_id ?  get_the_title($blog_page_id) : __( 'Post', 'tcd-seeed' );
	...
	// 各タクソノミーアーカイブ
    	 ...
    	 $title = $term_meta['meta_title'];
			 ...
			 $title = sprintf( __( 'Post list for %s', 'tcd-seeed' ), $term_name ) . ' | ' . $site_title;
		 }

	// 検索結果ページ
 	} elseif ( is_search() ) {
    if ( !empty( get_search_query() ) ) {
      $title = sprintf( __( 'Search result for %s', 'tcd-seeed' ), get_search_query() ) . ' | ' . $site_title;
    } else {
      $title = __( 'Search result', 'tcd-seeed' ) . ' | ' . $site_title;
    }

	// 日月アーカイブ
 	} elseif ( is_day() ) {
  	$title = sprintf( __( 'Archive for %s', 'tcd-seeed' ), get_the_time( __( 'F jS, Y', 'tcd-seeed' ) ) ) . ' | ' . $site_title;
 	} elseif ( is_month() ) {
  	$title = sprintf( __( 'Archive for %s', 'tcd-seeed' ), get_the_time( __( 'F, Y', 'tcd-seeed') ) ) . ' | ' . $site_title;
 	} elseif ( is_year() ) {
  	$title = sprintf( __( 'Archive for %s', 'tcd-seeed' ), get_the_time( __( 'Y', 'tcd-seeed') ) ) . ' | ' . $site_title;

	// 著者ページ
 	} elseif ( is_author() ) {
 		global $wp_query;
  	$curauth = $wp_query->get_queried_object();
    $author_name = sprintf( __( '%s blog list', 'tcd-seeed' ), $curauth->display_name );
...
function get_seo_description() {
  ...
  $site_description = get_bloginfo( 'description', 'display' );
    ...
    return sprintf( __( 'Archive for %s', 'tcd-seeed' ), get_the_time( __( 'F jS, Y', 'tcd-seeed' ) ) );
 	} elseif ( is_month() ) {
    return sprintf( __( 'Archive for %s', 'tcd-seeed' ), get_the_time( __( 'F, Y', 'tcd-seeed' ) ) );
 	} elseif ( is_year() ) {
    return sprintf( __( 'Archive for %s', 'tcd-seeed' ), get_the_time( __( 'Y', 'tcd-seeed' ) ) );
 	} elseif ( is_author() ) {
    global $wp_query;
    $curauth = $wp_query->get_queried_object();
    return sprintf( __( 'Archive for %s', 'tcd-seeed' ), esc_html( $curauth->display_name ) );
 	} elseif ( is_search() ) {
    return sprintf( __( 'Post list for %s', 'tcd-seeed' ), get_search_query() );
